...
## 🎯 SKU 判斷與條件執行 (ExecMode 4)

在 Centimani 腳本中，可以利用 SFIS 系統提供的 SKU 值來實現自動化的條件判斷和執行不同的測試流程或呼叫不同的外部程式。這對於多產品型號共用一套測試腳本的場景非常有用。

### **核心概念**

- **`*$(sfisSKU)`**: 這是 Centimani 腳本中用於獲取當前 DUT (Device Under Test) 的 SFIS SKU 值的變數。
- **`ExecMode: 4:PassStep,FailStep`**: 條件執行模式。
  - 如果當前步驟的 `Reply` 匹配成功，則跳轉到 `PassStep` 指定的步驟。
  - 如果當前步驟的 `Reply` 匹配失敗，則跳轉到 `FailStep` 指定的步驟。

### **範例：根據 SKU 執行不同的韌體更新 BAT 檔案**

假設您有兩種產品型號 (Q611GT 和 Q612EB)，它們需要執行不同的韌體更新 BAT 檔案。這兩個 BAT 檔案分別是 `BurnIN_firmware_checkDM_AT_port_Q611GT.bat` 和 `BurnIN_firmware_checkDM_AT_port_Q612EB.bat`。

#### **腳本設計**

| Phase | Title | Send | Reply | Waiting | ExecMode | Description |
|-------|-------|------|-------|---------|----------|-------------|
| 1 | Check Product SKU | :GetVal,"$sfisSKU" | @Q611GT | 3000 | 4:2,3 | 檢查 SFIS 系統中的 SKU 值。如果包含 "Q611GT" 則執行步驟 2 (Q611GT 韌體更新)，否則執行步驟 3 (Q612EB 韌體更新)。 |
| 2 | Execute Q611GT BAT | CmdConsole@call "Tools\\FW_IMAGE\\BurnIN_firmware_checkDM_AT_port_Q611GT.bat" | 0 | 5000 | 0 | 執行 Q611GT 產品的韌體更新 BAT 檔案。 |
| 3 | Execute Q612EB BAT | CmdConsole@call "Tools\\FW_IMAGE\\BurnIN_firmware_checkDM_AT_port_Q612EB.bat" | 0 | 5000 | 0 | 執行 Q612EB 產品的韌體更新 BAT 檔案。 |

#### **工作原理說明**

1. **步驟 1 (Check Product SKU):**
   - `Send: :GetVal,"$sfisSKU"`: 這個命令會從 SFIS 系統讀取當前 DUT 的 SKU 值。例如，如果 SKU 是 "MU310-Q611GT"，它會讀取 "MU310-Q611GT"。
   - `Reply: @Q611GT`: 腳本會檢查讀取的 SKU 值中是否包含字串 "Q611GT"。
   - `ExecMode: 4:2,3`:
     - 如果 `Reply` 匹配成功 (即 SKU 包含 "Q611GT")，腳本會跳轉到 `PassStep` 指定的步驟，也就是 **步驟 2**。
     - 如果 `Reply` 匹配失敗 (即 SKU 不包含 "Q611GT"，例如是 "Q612EB")，腳本會跳轉到 `FailStep` 指定的步驟，也就是 **步驟 3**。

2. **步驟 2 (Execute Q611GT BAT):**
   - 如果 SKU 是 Q611GT，此步驟會被執行。它會呼叫 `BurnIN_firmware_checkDM_AT_port_Q611GT.bat` 進行韌體更新。

3. **步驟 3 (Execute Q612EB BAT):**
   - 如果 SKU 不是 Q611GT (例如是 Q612EB)，此步驟會被執行。它會呼叫 `BurnIN_firmware_checkDM_AT_port_Q612EB.bat` 進行韌體更新。

#### **優點**

- **自動化判斷**：無需手動修改腳本，根據 SFIS 系統的 SKU 自動選擇執行路徑。
- **單一腳本**：一套腳本可以適用於多種產品型號，提高維護效率。
- **靈活性**：可以根據 SKU 執行任何不同的測試步驟或外部程式。

#### **注意事項**

- 確保 SFIS 系統正確設定並提供 SKU 值。
- `Reply` 欄位中的匹配字串應與您的 SKU 命名規則一致。
- `Tools\\FW_IMAGE\\` 路徑中的反斜線需要雙重轉義 (`\\\\`)，因為在 CSV 或某些環境中單個反斜線可能被解釋為轉義字符。

#### **重要修正說明 (2025-08-12)**

**⚠️ 錯誤用法警告**：
- ❌ `CmdConsole@echo *$(sfisSKU)` - 這種寫法在實際腳本中沒有使用過
- ✅ `:GetVal,"$sfisSKU"` - 這是腳本範例速查表中的標準用法

**腳本範例速查表驗證**：
- 所有實際腳本都使用 `:GetVal,"$變數名稱"` 格式
- 變數名稱格式：`$ISN`, `$G_VALO360_MODEL`, `$sfisSKU` 等
- 用途：讀取和驗證變數值，配合條件執行模式使用
...
| Phase | Title | Send | Reply | Waiting | ExecMode | Description |
|-------|-------|------|-------|---------|----------|-------------|
| 1 | Check Product SKU | :GetVal,"$sfisSKU" | @Q611GT | 3000 | 4:2,4 | 檢查 SKU，Q611GT 執行步驟 2，否則執行步驟 4 |
| 2 | Execute Q611GT BAT | CmdConsole@call "Tools\\FW_IMAGE\\Q611GT.bat" | 0 | 5000 | 0 | 執行 Q611GT 流程 |
| 3 | Check Q612EB SKU | :GetVal,"$sfisSKU" | @Q612EB | 3000 | 4:4,5 | 再次檢查 SKU，Q612EB 執行步驟 4，否則執行步驟 5 |
| 4 | Execute Q612EB BAT | CmdConsole@call "Tools\\FW_IMAGE\\Q612EB.bat" | 0 | 5000 | 0 | 執行 Q612EB 流程 |
| 5 | Execute Default BAT | CmdConsole@call "Tools\\FW_IMAGE\\Default.bat" | 0 | 5000 | 0 | 執行預設流程 |
```
...
### **2025-08-12 重要修正**
- **修正變數讀取方式**：將錯誤的 `CmdConsole@echo *$(sfisSKU)` 用法修正為正確的 `:GetVal,"$sfisSKU"`
- **腳本範例速查表驗證**：基於實際腳本分析，確認 `:GetVal` 是標準用法
- **錯誤用法警告**：明確標示 `CmdConsole@echo` 在變數讀取中的錯誤用法
- **標準格式統一**：所有 SKU 檢查相關範例都使用 `:GetVal,"$變數名稱"` 格式

**修正依據**：
- 腳本範例速查表中的實際腳本驗證
- KANGAROO 和 VALO360 專案腳本分析
- 實際腳本中 `:GetVal` 的廣泛使用

### **2025-08-12 Global.ini 深入分析**
- **完整欄位意義說明**：詳細解析 Settings 和 Scripts 區塊中每個參數的具體意義
- **網路配置欄位**：PCIP_1、DUTIP_1、DUT_UR1、PCUART_fixture 等網路和串口參數
- **設備配置欄位**：DEVID_1、LOAD_1、DUT_Login 等設備控制參數
- **儀器配置欄位**：NI6008_DEV1、DAQ_1、LED_Light1、LED_AZ1 等儀器識別參數
- **產品配置欄位**：Product_SKU、MFG_WIFI_MB、FW_VERSION 等版本管理參數
- **SKU 配置欄位**：sfisSKU、VALO360_5G、VALO360_WIFI、VALO360_NObattery 等產品識別參數
- **SIM 卡配置欄位**：SIM1、SIM2、SIM3 等網路測試參數
- **天線配置欄位**：Check _ANT1、Check _ANT2 等射頻測試參數
- **使用方式範例**：在 DUT Sheet 和測試腳本中的具體應用方法
- **設計理念說明**：中央配置中心的集中管理和環境適應性

**分析依據**：
- AI 表中的 enhanced_ini_settings.csv 詳細分析
- VALO360_Examples_Table.csv 和 VALO360_Command_Reference.csv 的實際應用
- 腳本範例速查表中的變數使用模式
- 實際腳本中的配置參數驗證
...
### **問題：ExecMode 欄位值無法正確顯示**

在創建包含 `ExecMode` 值的 CSV 檔案時，可能會遇到以下問題：

#### **症狀描述**
- CSV 檔案中的 `ExecMode` 欄位顯示為空
- 即使檔案內容包含正確的 `ExecMode` 值（如 `4:2,3`），在 Excel 中開啟時該欄位仍為空
- 這會導致腳本無法正確執行條件邏輯

#### **問題原因分析**
1. **CSV 解析器限制**：冒號 `:` 和逗號 `,` 在 CSV 格式中有特殊含義
2. **編碼問題**：UTF-8 編碼在某些 Excel 版本中可能無法正確識別
3. **欄位分隔符衝突**：複雜的 `ExecMode` 值（如 `4:2,3`）可能被誤解為多個欄位

#### **解決方案**

##### **方案 1：直接複製腳本內容**
```
不依賴 CSV 檔案，直接將腳本內容複製到 Centimani 腳本中：

| Phase | Title | Send | Reply | Waiting | ExecMode | Description |
|-------|-------|------|-------|---------|----------|-------------|
| 1 | Check Product SKU | :GetVal,"$sfisSKU" | @Q611GT | 3000 | 4:2,3 | 檢查 SKU 值 |
| 2 | Execute Q611GT BAT | CmdConsole@call "Tools\FW_IMAGE\Q611GT.bat" | 0 | 5000 | 0 | 執行 Q611GT |
| 3 | Execute Q612EB BAT | CmdConsole@call "Tools\FW_IMAGE\Q612EB.bat" | 0 | 5000 | 0 | 執行 Q612EB |
```
...
1. **測試 CSV 檔案**：創建後立即用記事本檢查內容
2. **備用方案**：準備多種格式的腳本檔案
3. **腳本驗證**：在 Centimani 中測試腳本邏輯
4. **文檔記錄**：記錄腳本設計邏輯，便於手動重建

#### **相關腳本範例**

**SKU 條件執行腳本（手動填入版本）：**
```excel
| Index_149 | Phase | Title | Send | Reply | Waiting | ExecMode | Description |
|-----------|-------|-------|------|-------|---------|----------|-------------|
| 1 | 1 | Check Product SKU | CmdConsole@echo *$(sfisSKU) | @Q611GT | 3000 | 4:2,3 | 檢查 SKU 值 |
| 2 | 2 | Execute Q611GT BAT | CmdConsole@call "Tools\FW_IMAGE\Q611GT.bat" | 0 | 5000 | 0 | 執行 Q611GT |
| 3 | 3 | Execute Q612EB BAT | CmdConsole@call "Tools\FW_IMAGE\Q612EB.bat" | 0 | 5000 | 0 | 執行 Q612EB |
```

#### **總結**

雖然 CSV 檔案在傳輸腳本內容時可能遇到 `ExecMode` 值顯示問題，但這不影響腳本的核心邏輯設計。建議：

1. **優先使用直接複製**：將腳本內容直接複製到 Centimani 中
2. **手動驗證**：確保所有欄位值都正確填入
3. **測試驗證**：在實際環境中測試腳本邏輯
4. **文檔備份**：保存腳本設計文檔，便於後續維護

---

## 📥 文件讀取與工具使用準則（2025-08-13）

- **先尋找既有工具**：讀取 Excel/PPT/CSV/TSV/INI 等檔案時，先檢查是否已有既有 Python 工具可用。
  - 既有工具清單：`universal_centimani_script_analyzer.py`（通用 Centimani 腳本/文件分析器）。
  - 若檔案屬於 Centimani 腳本/規格/速查表/訓練簡報，優先使用此工具。
- **無現成工具再開發**：若既有工具不支援該檔案型態或需求，再評估開發新工具（如以 pandas、openpyxl、python-pptx）。
- **持續擴充**：新開發的功能應合併回既有工具或同一目錄，並更新說明與使用文件，保持單一入口與可維護性。
- **輸出集中**：工具輸出（分析報告/JSON/CSV/Log）集中存放於與工具同層的 `output/` 目錄，便於後續查閱。
- **預設 TestID 規則**：若臨時不確定 TestID，預設填入 `B7PL025`，待後續以 `Test Item Code` 速查表確認後再更新。

### 推薦使用流程
1. 判斷檔案是否為 Centimani 相關（腳本/規格/速查表/教學）。
2. 是 → 使用 `universal_centimani_script_analyzer.py`；否 → 使用通用解析（pandas/openpyxl/python-pptx）。
3. 產生之結果一律輸出至 `output/`，並在必要時同步整理為 AI 表可引用的 Markdown 區塊。

### 相關檔案與位置
- 工具：`universal_centimani_script_analyzer.py`
- 輸出資料夾：`output/`
- 範例資料夾：`examples/`
...
- 2025-08-13：新增「文件讀取與工具使用準則」，規範讀取 Excel/PPT 先重用既有工具、無則再開發；新增預設 TestID = `B7PL025` 規則；統一輸出位置說明。